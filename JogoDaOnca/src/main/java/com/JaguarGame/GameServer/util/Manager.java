...
public class Manager {
  ...
  public int registerPlayer(String playerName) throws InterruptedException {
    if (this.players.isPlayerRegistered(playerName))
      return -1;
    if (this.players.isMaxPlayersReached())
      return -2;
    ...
    if (record != null) {
      ...
      if (record.getPlayerName2().equals(playerName))
        id = record.getPlayerId2();
  ...
  public int hasGame(int playerId) throws InterruptedException {
    ...
    if (g.hasPlayer1() && g.hasPlayer2()) {
      if (g.getPlayer1().equals(p))
        return 1; // Player is JAGUAR!
      if (g.getPlayer2().equals(p))
        return 2; // Player is DOGS!
  ...
  public int isMyTurn(int playerId) throws InterruptedException {
    ...
    if (g.hasWinner()) {
      ...
      if (g.isPlayer1(p) && g.getTurn() == PlayerType.Jaguar)
        return 1;
      if (g.isPlayer2(p) && g.getTurn() == PlayerType.Dog)
        return 1;
  ...
  public int sendMove(int playerId, int dogId, Direction direction) throws InterruptedException {
    ...
    if (g.getTurn() == PlayerType.Dog && p == g.getPlayer1())
      return -3;
    if (g.getTurn() == PlayerType.Jaguar && p == g.getPlayer2())
      return -3;

    // Valida se usou o animal certo
    if (dogId == -1 && p == g.getPlayer2())
      return -4;
    if (dogId != -1 && p == g.getPlayer1())
      return -4;
  ...
  public String getOpponent(int playerId) throws InterruptedException {
    ...
    if (g.isPlayer1(p))
      return g.getPlayer2().getName();
    else
      return g.getPlayer1().getName();
  }
}