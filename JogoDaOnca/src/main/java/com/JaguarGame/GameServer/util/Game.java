...
import com.JaguarGame.GameServer.game.Board;
import com.JaguarGame.GameServer.game.Piece;

public class Game {
  private static final int MOVE_TIMEOUT = 1000 * 10; // 10 seconds
  ...
  public void checkWinner() {
    if (this.board.isJaguarWinner())
      this.winner = PlayerType.Jaguar;
    if (this.board.isDogsWinner())
  ...
  public boolean isWinner(Player p) {
    if (this.winner == null)
      return false;
    if (this.isPlayer1(p) && this.winner == PlayerType.Jaguar)
      return true;
    if (this.isPlayer2(p) && this.winner == PlayerType.Dog)
      return true;
    return false;
  }
  ...
  public int move(int dogId, Direction direction) {
    ...
    if (!this.board.canMove(piece, direction))
      return 0;
